...
const apiCall = async (endpoint: string, options: RequestInit = {}) => {
  ...
  try {
    ...
    if (error.name === 'TypeError' && error.message.includes('fetch')) {
      throw new Error(`خطا در اتصال به سرور. لطفاً مطمئن شوید که backend در دسترس است: ${API_BASE_URL}`)
    }
    
    // Handle CORS errors
    if (error.message && error.message.includes('CORS')) {
      throw new Error('خطای CORS: لطفاً domain frontend را در تنظیمات backend اضافه کنید')
    }
