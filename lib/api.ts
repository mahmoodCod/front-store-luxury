...
const apiCall = async (endpoint: string, options: RequestInit = {}) => {
  ...
  try {
    ...
    if (error.name === 'AbortError') {
      const timeoutError = 'درخواست به سرور timeout شد. لطفاً دوباره تلاش کنید.'
    ...
    if (error.name === 'TypeError' && (error.message.includes('fetch') || error.message.includes('Failed to fetch'))) {
      ...
      const errorMsg = 'خطا در اتصال به سرور. ممکن است backend در حال راه‌اندازی باشد یا مشکل CORS وجود داشته باشد.'
...
export const productApi = {
  ...
  createProduct: async (data: FormData) => {
    ...
    console.log('[API] Response text:', text.substring(0, 500)) // First 500 chars
    ...
    if (!response.ok) {
      ...
      console.error('[API] Product creation failed:', errorMessage)
