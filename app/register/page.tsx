...
export default function RegisterPage() {
  ...
  const handleSendCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = `${API_BASE_URL}/api/v1/auth/send`
      console.log('[Register] Sending OTP request to:', url)
      ...
      if (error?.message?.includes('timeout') || error?.name === 'AbortError') {
        toast.error('درخواست timeout شد. لطفاً دوباره تلاش کنید.')
      } else if (error?.message?.includes('Failed to fetch') || error?.message?.includes('fetch') || error?.message?.includes('اتصال به سرور')) {
        toast.error('خطا در اتصال به سرور. لطفاً چند لحظه صبر کنید و دوباره تلاش کنید.')
        console.error('[Register] Network error - Backend may be sleeping or CORS issue')
      } else if (error?.message?.includes('CORS')) {
        toast.error('خطای CORS. لطفاً با پشتیبانی تماس بگیرید.')
      } else {
  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('[Register] Verifying OTP request to:', url)
      ...
      console.log('[Register] Verify response ok:', response.ok)
      
      // ابتدا response رو به text تبدیل کن
      const responseText = await response.text()
      console.log('[Register] Verify response text:', responseText)
      
      // If status is not ok, log error
      if (!response.ok) {
        console.error('[Register] Verify error response:', {
